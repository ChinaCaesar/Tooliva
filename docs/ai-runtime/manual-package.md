...
The desktop app installs the runtime only after the user selects the local zip file.
...
`ai-runtime.zip` must contain one of these layouts:
...
1. User downloads `ai-runtime.zip`.
...
1. `ai-runtime.zip` extracts successfully.
